...
const api = `https://social-media.piyushsingh6.repl.co`;
...
export async function apiCall(type, endPoint, body) {
  switch (type) {
    case "GET":
      try {
        let { status, data } = await axios.get(`${api}/${endPoint}`);
    ...
    case "POST":
      try {
        let { status, data } = await axios.post(`${api}/${endPoint}`, body);
    ...
    case "DELETE":
      try {
        let { status, data } = await axios.delete(`${api}/${endPoint}`, {
          data: body,
        });
        if (status === 200) {
          return { success: true, data: data, message: data.message };
